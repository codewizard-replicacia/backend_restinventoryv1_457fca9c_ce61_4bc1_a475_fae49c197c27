package com.apps.restinventory.nameBuilder;
...
public class APINameBuilder implements JPAEdmNameBuilder {
	...
	public String buildEntitySetName(final String entityTypeName) {
    	...
    	eSetName = "Reorder".equals(entityTypeName) ? "Reorders" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "ProductCategory".equals(entityTypeName) ? "ProductCategories" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "PurchaseOrder".equals(entityTypeName) ? "PurchaseOrders" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "ProductStore".equals(entityTypeName) ? "ProductStores" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	...
    	eSetName = "ProductCatalogue".equals(entityTypeName) ? "ProductCatalogues" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "InventoryStock".equals(entityTypeName) ? "InventoryStocks" : defaultNameBuilder.buildEntitySetName(entityTypeName);
        return eSetName;
    }
